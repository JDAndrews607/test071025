...
def generate_IC_NoisyData(datapoints = 10000, noise = 0.05):
    ...
    def genGaussianPeak(x, mu, sigma, amplitude):
        curve = amplitude * np.exp(-0.5 * ((x - mu) / sigma)**2)
        return curve
...
def peakFind_EMG(timestamp, CD_Signal, targetTime, _lambda = 0.01, peakDeviation = 1):     #Using exponentially modified Gaussian equation
    ...
    for i in range(peakIterator, 0, -1):
        if CD_Signal[i] <= 0.005 * maxPeak:  #curve stops at 0.5% of peak height
...
def errorFunction_EMG(timestamp, CD_Signal, area, mu, sigma, _lambda):
    error = 0.0
    for i in range(len(timestamp)):
        modelVal = area * exponentialModifiedGaussian_EMG(timestamp[i],  mu, sigma, _lambda)
...
def fit_EMG(timestamp, CD_Signal, init_area, init_mu, init_sigma, init_lambda, steps = 1000, step_size = 0.01):
    ...
    bounds = [  #bounded values for each parameter
        (0, init_mu),     #for mu
        (0, init_sigma),   #for sigma
        (0, init_lambda),   #for lambda
        (0, init_area)   #for area
    ]
    ...
    for i in population:
        ...
        for j in range(len(timestamp)):
            y_pred = exponentialModifiedGaussian_EMG(timestamp[j], mu, sigma, _lambda, area)
    ...
    for step in range(0, steps):    #1000 generations
        ...
        diffEvo_Params = population[bestIndex]    #obtains the most accurate parameters from differential evolution
...
def exponentialModifiedGaussian_EMG(x, mu, sigma, _lambda, area = 1):
    ...
    while count < 10: #takes series to ten terms:
        ...
        count += 1
    erfc = 1 - 2 / pi**(1/2) * erf
    #EMG_final = EMG_Front * erfc_Abramowitz__Stegun(erfc_term)      #odd shape, maybe don't use...
    #EMG_final = EMG_Front * erfc        #third degree polynomial somewhat mimicking curve
    EMG_final = area * EMG_Front * math.erfc(erfc_term)        #straight line at y=0
